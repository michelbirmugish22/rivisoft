...
def index(req):  
    ...
    pdf = SimpleDocTemplate(buffer, pagesize=portrait(A4))
    page = []
    large, haut = A4
    page.append(Image("media\images\logo_hnr_gerance.png",large-120,80))
    page.append(Paragraph(f"BREAKFAST LIST DU {Account.objects.last().account_date_room.strftime("%d/%m/%Y")}", style1))
